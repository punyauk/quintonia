// CHANGES
string TXT_ADD_KWH="Add energy";
string TXT_KWH_FOUND="Found energy, emptying...";
...
//
float   VERSION = 5.2;    // 12 March 2022
...
string SUFFIX = "L1";
//
list    energyChannels = [-321321, -449718];  // [Electric, Gas]
    ...
    if (llGetInventoryType("config") == INVENTORY_NOTECARD)
    {
        ...
        for (i=0; i < llGetListLength(lines); i++)
        {
            ...
            if (llGetSubString(line, 0, 0) != "#")
            {
                ...
                {
                    // Currently energy types are:  0=electric  1=gas
                    if (llToLower(val) == "gas") energyType = 1; else energyType = 0;
                }
...
fireOff()
{
    if (burning == TRUE)
    {
        ...
        llSetLinkPrimitiveParamsFast(getLinkNum(lightObj), [ PRIM_COLOR, face, <0,0,0> , trans, 
...
fireOn()
{
    if (burning == FALSE)
    {
        ...
        {
            integer face = 3;
            if (lightObj == "Fire") face = ALL_SIDES; 
            llSetLinkPrimitiveParamsFast(getLinkNum(lightObj), [ PRIM_COLOR, face, <0.789, 0.631, 0.211> , 1.0,
...
setSupplier()
{
    if (onGrid == FALSE)
    {
        llSetLinkPrimitiveParamsFast(getLinkNum("Indicator"), [PRIM_COLOR, ALL_SIDES, <0.502, 0.502, 1.000>, 1.0]);
    }
    else
    {
        llSetLinkPrimitiveParamsFast(getLinkNum("Indicator"), [PRIM_COLOR, ALL_SIDES, <0.000, 0.502, 0.502>, 1.0]);
    }
}
